...
let mostrar_tabla = async () => {
    ...
    let libreriaActiva=sessionStorage.getItem('id')
    ...
    for (let index = 0; index < listaUsuarios.length; index++) {
       if (listaUsuarios[index]._id==libreriaActiva) {
           libreriaActiva=listaUsuarios[index].identificacion;
       }         
    }

    for (let i = 0; i < lista_librerias.length; i++) {
        console.log(libreriaActiva)
        if (libreriaActiva==lista_librerias[i].id) {
