...
let validarSignInFoot = (pusuario,pcontrasena)=>{
    let error = false;

    if (pusuario=='') {
        error=true;
        input_usuariofooter.classList.add('input_error');

    } else {
        input_usuariofooter.classList.remove('input_error');
    }

    if (pcontrasena == '') {
        error=true;
        input_contrasenafooter.classList.add('input_error');

    } else {
        input_contrasenafooter.classList.remove('input_error');
...
let inicioSesionFooter = async()=>{
    let tusuario = input_usuariofooter.value;
    let tcontrasena=input_contrasenafooter.value;
    let error =  validarSignInFoot(tusuario,tcontrasena);
    let errorCredenciales = await iniciar_Sesion(tusuario,tcontrasena);
    let modalContent=document.querySelector('#signinpopup');
    console.log(errorCredenciales);

    if (error==true) {
        footer_error.style.display="block";
